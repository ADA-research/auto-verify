...
For a comprehensive overview, use: auto-verify help
    ...
    parser.add_argument("--version", action="version", version=f"auto-verify {__version__}")
    ...
    install_parser.add_argument(
        "--env",
        choices=["conda", "venv", "auto"],
        default="conda",
        help="Environment management strategy (default: conda)",
    )
    install_parser.add_argument(
        "--force-venv", action="store_true", help="Force use of venv even if conda is preferred"
    )
    install_parser.add_argument(
        "--verifier-version",
        type=str,
    ...
    install_parser.add_argument(
        "--conda-env-name", type=str, help="Custom conda environment name (overrides default __av__{verifier} naming)"
    )
...
"""
    ...
    uninstall_parser.add_argument("verifiers", nargs="+", help="Verifiers to uninstall")
    uninstall_parser.add_argument(
        "--env", choices=["conda", "venv", "both"], default="both", help="Which installation type to remove"
    )

    # Delete command (alias for uninstall)
    delete_parser = subparsers.add_parser("delete", help="Delete verifiers (alias for uninstall)")
    delete_parser.add_argument("verifiers", nargs="+", help="Verifiers to delete")
    ...
    list_parser.add_argument(
        "--env", choices=["conda", "venv", "both"], default="both", help="Which installation type to list"
    )
    list_parser.add_argument(
        "--verbose", "-v", action="store_true", help="Show detailed information about each verifier"
    )

    # Check command
    check_parser = subparsers.add_parser("check", help="Check verifier status and health")
    check_parser.add_argument(
        "--env", choices=["conda", "venv", "both"], default="both", help="Which installation type to check"
    )
    check_parser.add_argument("--verbose", "-v", action="store_true", help="Show detailed status information")

    # Clean command
    clean_parser = subparsers.add_parser("clean", help="Clean up failed verifier installations")
    clean_parser.add_argument("verifiers", nargs="+", help="Verifiers to clean up")
    clean_parser.add_argument(
        "--env", choices=["conda", "venv", "both"], default="both", help="Which installation type to clean"
    )

    # Versions command
    versions_parser = subparsers.add_parser("versions", help="Show available versions for verifiers")
    versions_parser.add_argument("verifier", help="Name of the verifier to check versions for")
    versions_parser.add_argument("--branch", type=str, default=None, help="Branch to check (default: main/master)")
    ...
    set_env_parser.add_argument("strategy", choices=["conda", "venv", "auto"], help="Environment management strategy")

    # Config set-install-path
    set_path_parser = config_subparsers.add_parser("set-install-path", help="Set custom installation path")
    set_path_parser.add_argument("path", type=str, help="Custom installation path (use 'default' to reset to default)")

    # Config set-gpu
    set_gpu_parser = config_subparsers.add_parser("set-gpu", help="Set GPU preference")
    set_gpu_parser.add_argument("prefer", choices=["true", "false"], help="Whether to prefer GPU-enabled installations")

    # Config set-timeout
    set_timeout_parser = config_subparsers.add_parser("set-timeout", help="Set default timeout")
    set_timeout_parser.add_argument("seconds", type=int, help="Default verification timeout in seconds")

    # Config set-log-level
    set_log_parser = config_subparsers.add_parser("set-log-level", help="Set logging verbosity")
    set_log_parser.add_argument("level", choices=["DEBUG", "INFO", "WARNING", "ERROR"], help="Logging verbosity level")
    ...
    set_verbose_parser.add_argument(
        "verbose", choices=["true", "false"], help="Whether to show detailed output during installation"
    )
    ...
    set_fallback_parser.add_argument(
        "allow", choices=["true", "false"], help="Whether to allow falling back to conda if venv+uv is not available"
    )
    ...
    set_uv_parser.add_argument(
        "require", choices=["true", "false"], help="Whether to require uv when using venv strategy"
    )
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if latest_commit != repo_info.commit_hash:
                print(
                    f"  Note: Default commit is {len(repo_info.commit_hash)} characters, "
